...
@JvmOverloads constructor(
    ...
    private fun addAsPKDStoreCSCACertStore(uri: URI) {
        ...
        val port = uri.port
        val params =
            if (port < 0) PKDCertStoreParameters(server) else PKDCertStoreParameters(server, port)
        val cscaParams =
            if (port < 0) PKDMasterListCertStoreParameters(server) else PKDMasterListCertStoreParameters(
                server,
                port
            )
